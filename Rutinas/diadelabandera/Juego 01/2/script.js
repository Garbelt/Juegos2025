...
function updateClockDisplay() {
    ...
    function getRandomImage() {
        const totalImages = 2; // 👈 CAMBIAR según la cantidad real de imágenes
...
function createPuzzle() {
    ...
    const emptyPiece =
        document.createElement("div");
    emptyPiece.classList.add(
        "piece",
        "empty"
    );
    emptyPiece.style.order =
        size * size - 1;
...
function endGame() {
    ...
    firebase.database().ref("games").push().set({
        ...
        game: "MBELG_rmpc01",
        puntaje: score,
        title: "RompeCabezas de Manuel Belgrano",
        usuario: usuario
    })
