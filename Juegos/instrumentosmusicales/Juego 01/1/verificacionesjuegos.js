...
window.addEventListener("load", async () => {
    ...
    const juegoCode = "DTOS_sei";
    const rutinaCode = "DTOS_sei";

const modoLocal =
    localStorage.getItem("modolocal") === "SI";

if (modoLocal) {

    // Mantener control de paso
    ...
    // Habilitar directamente el juego
...
// ⭐⭐⭐ Verificación previa: si es DOCENTE, saltar habilitación ⭐⭐⭐
const esUsuarioDocente = await esDocente(uid);

if (esUsuarioDocente) {
    console.log("Acceso docente: se omite verificación de habilitación.");
    startButton.classList.remove("disable-clicks");
    return; 
}
...
async function esDocente(uid) {
    try {
        const snapshot = await firebase.database()
            .ref(`usuarios/${uid}/rol`)
            .once("value");
        return snapshot.exists() && snapshot.val() === "docente";
    } catch (err) {
        console.error("Error verificando rol docente:", err);
        return false;
    }
}
