...
const words = ["CHACARERA", "GATO", "ESCONDIDO", "CARNAVALITO", "CHAMAMÉ", "BAILECITO", "HUELLA", "MALAMBO", "VIDALA", "BAGUALA", "CHAMARRITA", "TRIUNFO", "HUAYNO"];
...
function showFoundMessage() {
  ...
  // 1️⃣ Obtener palabra
  const palabra = document.getElementById("palabras").textContent;
  // 2️⃣ Leer SOLO la palabra
  hablar(palabra);
  // 3️⃣ Sumar puntaje
  score += 20;
  document.getElementById("puntaje").textContent = `${score}`;
  // 4️⃣ Mostrar mensaje y continuar
  showGameMessage(
    "PALABRA ENCONTRADA",
    3000,
    "sound/PalabraEncontrada.mp3",
    () => {
      if (gameEnded) return;
      checkForBonus();
...
function saveGameScore() {
  ...
  saveGameResultToFirebase(score, "FOLK_sdl01", "Hallar en la sopa de letras el nombre de estilos folclóricos argentinos", usuario);
}
...
function leerReferenciaConResaltado(){
    if (gameEnded) return;
    const palabraEl = document.getElementById("palabras");
    if (!palabraEl) return;
    const palabra = palabraEl.textContent.trim();
    if (!palabra) return;
    hablar("Palabra a encontrar", {
        onEnd: () => {
            palabraEl.classList.add("resaltado-lectura");
            hablar(palabra, {
                onEnd: () => {
                    palabraEl.classList.remove("resaltado-lectura");
                }
            });
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    const referencia = document.getElementById("referencia-palabra");
    if (referencia){
        referencia.style.cursor = "pointer";
        referencia.addEventListener("click", () => {
            leerReferenciaConResaltado();
        });
    }
});
